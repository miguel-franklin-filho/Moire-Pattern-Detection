...
def main(positiveImagePath, negativeImagePath, train):
...
def transformExtension(path):
    return path.replace(".jpg", ".tiff").replace(".jpeg", ".tiff")
...
def transformImageAndSave(image, f, customStr, path):
    ...
    fLL = transformExtension((f.replace(fileName, fileName + "_" + customStr + "LL")))
    fLH = transformExtension((f.replace(fileName, fileName + "_" + customStr + "LH")))
    fHL = transformExtension((f.replace(fileName, fileName + "_" + customStr + "HL")))
    fHH = transformExtension((f.replace(fileName, fileName + "_" + customStr + "HH")))
...
def augmentAndTrasformImage(f, mainFolder, trainFolder):
    try:
        ...
        print(
            "Error: Couldnt read the file {}. Make sure only images are present in the folder".format(
                f
            )
        )
        return None
...
def createTrainingData(positiveImagePath, negativeImagePath):
    ...
    positiveImageFiles = [
        f for f in listdir(positiveImagePath) if (isfile(join(positiveImagePath, f)))
    ]
    negativeImageFiles = [
        f for f in listdir(negativeImagePath) if (isfile(join(negativeImagePath, f)))
    ]
...
def parse_arguments(argv):
    ...
    parser.add_argument(
        "negativeImages", type=str, help="Directory with negative (Normal) images."
    )
...
if __name__ == "__main__":
    ...
    main(args.positiveImages, args.negativeImages, args.train == 0)
